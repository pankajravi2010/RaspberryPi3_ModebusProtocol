...
int main(int argc, char *argv[])
{
    ...
    modbus_t *ctx = NULL;
    int i;
    uint16_t regis_count=0;
    uint16_t Data_buffer[DATA_BUFFER_SIZE];
    uint8_t value;
    int nb_points;
    int rc;
    float real;
    uint32_t old_response_to_sec=100000;
    uint32_t old_response_to_usec=1000000;
    uint32_t new_response_to_sec=100000;
    uint32_t new_response_to_usec=100000;
    uint32_t old_byte_to_sec=1000;
    uint32_t old_byte_to_usec=1000;
    int use_backend =RTU;//pankaj;
    int success = FALSE;
    int old_slave;
...
/*    if (argc > 1) {
    ...
    */
    ...
    if (use_backend == TCP) {
     ...
     //  ctx = modbus_new_rtu("/dev/ttyUSB0", 19200, 'N', 8, 1);   // with usb in rasppi
	ctx = modbus_new_rtu("/dev/ttyUSB0", 19200, 'N', 8, 1);   // NEED TO DEACTIVATED USED BY VASU SIR FOR TESTING.
           modbus_set_debug(ctx,TRUE);
        printf("** pankaj date 18/04/2018 clovity usart **\n");
    }
    if (ctx == NULL) {
    ...
    if (use_backend == RTU) {
        modbus_set_slave(ctx, 1/*SERVER_ID*/);
        printf("\n slave id set:1 \n");//for debugging 
    }
  ...
  //  printf("** UNIT TESTING **\n");
    ...
    printf("================HOLDING REGISTER READ/WRITE : STARTED=============\n");
   

//SECTION:-1 (ONLY READ MULTIPLE PERAMETERS SAMULTENIUOSLY)--->>THIS CODE IS READING FOR DATA
//=============================================================================================================================================================
             
//----------ONLY READING ROUTINE OF MULTIPLE PERAMETERS AT SAMULTENIOUSLY START HERE----------
/* while(1)//WHILE_1 START HERE
{
    memset(tab_rp_registers, 0, nb_points * sizeof(uint16_t));
     
       rc = modbus_write_register(ctx, (46), 0xA109);  //FOR HYSTERIS
     
   
      ASSERT_TRUE(rc == 1, "");
      usleep(2000000);//AT THIS DELAY EVERYTHING IS FINE
    
  

        rc = modbus_read_registers(ctx, (47),
    	                               1, tab_rp_registers);       	
    	Data_buffer[regis_count]=tab_rp_registers[0];

        ASSERT_TRUE(rc == 1, "FAILED (nb points %d)\n", rc);   
       printf("HYSTERIS(0xA109): 0x%X , addr=%d  \n",tab_rp_registers[0],(47));//for debugging
       usleep(1600000);//AT THIS DELAY EVERYTHING IS FINE

//----------------------------------------------------------------------------------------------------------------------------
     memset(tab_rp_registers, 0, nb_points * sizeof(uint16_t));
     
       rc = modbus_write_register(ctx, (46), 0xA108); //FOR THRESOLD
      ...
      usleep(2000000);//AT THIS DELAY EVERYTHING IS FINE
    	...
    	Data_buffer[regis_count]=tab_rp_registers[0];
        ASSERT_TRUE(rc == 1, "FAILED (nb points %d)\n", rc);   
       printf("THRESOLD(0xA108): 0x%X , addr=%d  \n",tab_rp_registers[0],(47));//for debugging

      usleep(1600000);//AT THIS DELAY EVERYTHING IS FINE
//---------------------------------------------------------------------------------------------------------------------------
       memset(tab_rp_registers, 0, nb_points * sizeof(uint16_t));
     
       rc = modbus_write_register(ctx, (46), 0xA107);//FOR SAMPLE LOW 
    
      ASSERT_TRUE(rc == 1, "");
      usleep(2000000);//usleep(400000);usleep(400000);usleep(400000);usleep(400000);//AT THIS DELAY EVERYTHING IS FINE
    	...
    	Data_buffer[regis_count]=tab_rp_registers[0];
        ASSERT_TRUE(rc == 1, "FAILED (nb points %d)\n", rc);   
        printf("SAMPLE LOW(0xA107): 0x%X , addr=%d  \n",tab_rp_registers[0],(47));//for debugging
...
//---------------------------------------------------------------------------------------------------------------------------
        ...
        rc = modbus_write_register(ctx, (46), 0xA106);//FOR SAMPLE HIGH 
    	...
    	Data_buffer[regis_count]=tab_rp_registers[0];
        ASSERT_TRUE(rc == 1, "FAILED (nb points %d)\n", rc);   
        printf("SAMPLE HIGH(0xA106): 0x%X , addr=%d  \n",tab_rp_registers[0],(47));//for debugging
...
//---------------------------------------------------------------------------------------------------------------------------
        ...
        rc = modbus_write_register(ctx, (46), 0xA104);//FOR REPORT RATE 
    	...
    	Data_buffer[regis_count]=tab_rp_registers[0];
        ASSERT_TRUE(rc == 1, "FAILED (nb points %d)\n", rc);   
        printf("REPORT RATE(0xA104): 0x%X , addr=%d  \n",tab_rp_registers[0],(47));//for debugging
...
//---------------------------------------------------------------------------------------------------------------------------
        ...
        rc = modbus_write_register(ctx, (46), 0xA103);//FOR SAMPLE  RATE 
    	...
    	Data_buffer[regis_count]=tab_rp_registers[0];
        ASSERT_TRUE(rc == 1, "FAILED (nb points %d)\n", rc);   
        printf("SAMPLE RATE(0xA103): 0x%X , addr=%d  \n",tab_rp_registers[0],(47));//for debugging
...
//---------------------------------------------------------------------------------------------------------------------------
        ...
        rc = modbus_write_register(ctx, (46), 0xA115);//FOR BASE LINE FILTER
    	...
    	Data_buffer[regis_count]=tab_rp_registers[0];
        ASSERT_TRUE(rc == 1, "FAILED (nb points %d)\n", rc);   
        printf("BASE LINE FILTER(0xA115): 0x%X , addr=%d  \n",tab_rp_registers[0],(47));//for debugging
...
*/
//----------ONLY READING ROUTINE OF MULTIPLE PERAMETERS AT SAMULTENIOUSLY END HERE----------
            //SECTION:-1 END HERE.
//==================================================================================================================



                  //SECTION:-2 (WRITE AND READ SAMULTENIOUSLY OF MULTIPLE PERAMETERS)
//==================================================================================================================                  
//----------------WRITE DATA AND READ DATA SAMULTENIOUSLY TO MODBUS HOLDING REGISTER START HERE-----------------------

//============================FOR SAMPLE LOW==============================================  
       //----WRITE PROCESS--------------   
		memset(tab_rp_registers, 0, nb_points * sizeof(uint16_t));
	       
	       rc = modbus_write_register(ctx, (15), 0x001C); //reg_16  //DATA 
	       usleep(2000000);
	       ASSERT_TRUE(rc == 1, "");         
	       rc = modbus_write_register(ctx, (14), 0x8107); //reg_15 //WRITE SAMPLE LOW  
	       usleep(2000000);
      	       ASSERT_TRUE(rc == 1, "");
     
       	//--READ ABOVE DATA PROCESS------
	       memset(tab_rp_registers, 0, nb_points * sizeof(uint16_t));

	       rc = modbus_write_register(ctx, (14), 0xA107); //reg_15 
	       usleep(2000000);
	       ASSERT_TRUE(rc == 1, "");
	     
	       rc = modbus_read_registers(ctx, (15),//reg_16 READ SAMPLE LOW
	    	                               1, tab_rp_registers);     	
	       Data_buffer[regis_count]=tab_rp_registers[0];
	       ASSERT_TRUE(rc == 1, "FAILED (nb points %d)\n", rc);   
	       printf("READ SAMPLE LOW: 0x%X , addr=%d  \n",tab_rp_registers[0],(15));//for debugging
	      usleep(20000);   

//============================FOR SAMPLE HIGH==============================================
   	//--WRITE PROCESS---------------------
	       memset(tab_rp_registers, 0, nb_points * sizeof(uint16_t));  
	       rc = modbus_write_register(ctx, (15), 0x006C); //reg_16 //DATA
	   
	      usleep(2000000);
	      ASSERT_TRUE(rc == 1, "");
	       
	       rc = modbus_write_register(ctx, (14), 0x8106); //reg_15 //WRITE SAMPLE HIGH  
	      usleep(2000000);
	      ASSERT_TRUE(rc == 1, "");  
	...
	//--READ ABOVE DATA PROCESS---------- 
	       ...
	       rc = modbus_write_register(ctx, (14), 0xA106); //reg_15
	       usleep(2000000);
	       ASSERT_TRUE(rc == 1, "");
	    
	       rc = modbus_read_registers(ctx, (15),//reg_16 READ SAMPLE HIGH
	    	                               1, tab_rp_registers);    	
	       Data_buffer[regis_count]=tab_rp_registers[0];
	       ASSERT_TRUE(rc == 1, "FAILED (nb points %d)\n", rc);   
	       printf("READ SAMPLE HIGH: 0x%X , addr=%d  \n",tab_rp_registers[0],(15));//for debugging
	     usleep(20000);
...
//============================FOR THRESOLD ==============================================
	//--WRITE PROCESS---------------------
	      ...
	      rc = modbus_write_register(ctx, (15), 0x000B); //reg_16 //DATA
	      usleep(2000000);
	      ASSERT_TRUE(rc == 1, "");
	     
	      rc = modbus_write_register(ctx, (14), 0x8108); //reg_15 WRITE THRESOLD 
	      usleep(2000000);
	      ASSERT_TRUE(rc == 1, "");

	//--READ ABOVE DATA PROCESS------------
	       memset(tab_rp_registers, 0, nb_points * sizeof(uint16_t));
	       rc = modbus_write_register(ctx, (14), 0xA108); //reg_15
	       usleep(2000000);
	       ASSERT_TRUE(rc == 1, "");
	       rc = modbus_read_registers(ctx, (15),//reg_16 READ THRESOLD
	    	                               1, tab_rp_registers);       	
	       Data_buffer[regis_count]=tab_rp_registers[0];
	       ASSERT_TRUE(rc == 1, "FAILED (nb points %d)\n", rc);   
	       printf("READ THRESOLD: 0x%X , addr=%d  \n",tab_rp_registers[0],(15));//for debugging
	       usleep(20000);
//============================FOR HYSTERISIS ==============================================
	//--WRITE PROCESS---------------------
	      memset(tab_rp_registers, 0, nb_points * sizeof(uint16_t));

	      rc = modbus_write_register(ctx, (15), 0x000D); //reg_16 //DATA
	      usleep(2000000);
	      ASSERT_TRUE(rc == 1, "");
	     
	      rc = modbus_write_register(ctx, (14), 0x8109); //reg_15 WRITE HYSTERISIS 
	      usleep(2000000);
	      ASSERT_TRUE(rc == 1, "");

	//--READ ABOVE DATA PROCESS------------
	       memset(tab_rp_registers, 0, nb_points * sizeof(uint16_t));
	       rc = modbus_write_register(ctx, (14), 0xA109); //reg_15
	       usleep(2000000);
	       ASSERT_TRUE(rc == 1, "");
	       rc = modbus_read_registers(ctx, (15),//reg_16 READ HYSTERISIS
	    	                               1, tab_rp_registers);       	
	       Data_buffer[regis_count]=tab_rp_registers[0];
	       ASSERT_TRUE(rc == 1, "FAILED (nb points %d)\n", rc);   
	       printf("READ HYSTERISIS: 0x%X , addr=%d  \n",tab_rp_registers[0],(15));//for debugging
	       usleep(20000);

//============================FOR BASE LINE FILTER ==============================================
	//--WRITE PROCESS---------------------
	      memset(tab_rp_registers, 0, nb_points * sizeof(uint16_t));

	      rc = modbus_write_register(ctx, (15), 0x000E); //reg_16 //DATA
	      usleep(2000000);
	      ASSERT_TRUE(rc == 1, "");
	     
	      rc = modbus_write_register(ctx, (14), 0x8115); //reg_15 WRITE BASE LINE FILTER
	      usleep(2000000);
	      ASSERT_TRUE(rc == 1, "");

	//--READ ABOVE DATA PROCESS------------
	       memset(tab_rp_registers, 0, nb_points * sizeof(uint16_t));
	       rc = modbus_write_register(ctx, (14), 0xA115); //reg_15
	       usleep(2000000);
	       ASSERT_TRUE(rc == 1, "");
	       rc = modbus_read_registers(ctx, (15),//reg_16 READ BASE LINE FILTER
	    	                               1, tab_rp_registers);       	
	       Data_buffer[regis_count]=tab_rp_registers[0];
	       ASSERT_TRUE(rc == 1, "FAILED (nb points %d)\n", rc);   
	       printf("READ BASE LINE FILTER: 0x%X , addr=%d  \n",tab_rp_registers[0],(15));//for debugging
	       usleep(20000);

    
    while(1);

//----------------WRITE DATA AND READ DATA, SAMULTENIOUSLY TO MODBUS HOLDING REGISTER END HERE-----------------------
           //SECTION:-2 END HERE.
//====================================================================================================================


 //SECTION:-3 (READ SITE SURVEY PERAMETERS) STARTS FROM HERE 
//=============================================================================================================================================================       
    while(1)
     {
     /*   memset(tab_rp_registers, 0, nb_points * sizeof(uint16_t));    // This also working code .
     
       rc = modbus_write_register(ctx, (14), 0x2002); //FOR ENTER INTO SITE SURVEY MODE INST=0X2002 AND FOR EXIT FROM SITE SURVEY MODE =0X2000. 
     // printf("\nvalue rc: %d conter val=%d \n",rc,regis_count);//for debugging
     // usleep(400000);
      ASSERT_TRUE(rc == 1, "");
     usleep(400000);usleep(400000);usleep(400000);//usleep(400000);usleep(400000);usleep(400000);usleep(400000);usleep(400000); 
    
   // printf("MODBUS WRITE TO REGISTER :SUCCESSFULL\n");//for debugging

         	rc = modbus_read_registers(ctx, (7),
    	                               1, tab_rp_registers);// Base_Addres 0x40000-->>262144        	
    	Data_buffer[regis_count]=tab_rp_registers[0];
        ASSERT_TRUE(rc == 1, "FAILED (nb points %d)\n", rc);   
       printf("SITE SURVEY: 0x%X , addr=%d  \n",tab_rp_registers[0],(7));//for debugging
      usleep(400000);usleep(400000);//usleep(400000);usleep(400000);usleep(400000);usleep(400000);usleep(400000);usleep(400000); 
       */
//==============================================================================================================================================
        memset(tab_rp_registers, 0, nb_points * sizeof(uint16_t));
     
       rc = modbus_write_register(ctx, (14), 0x2000); //FOR ENTER INTO SITE SURVEY MODE INST=0X2001 AND FOR EXIT FROM SITE SURVEY MODE =0X2000. 
                                                      //for Node1=0x2001,Node2=0x2002,Node3=0x2003.........and  exit from survey mode when data=0x2000 at only 							      //at Resister15(14 in code) working code ,testing-->>OK as on 28/05/2018 at 06:58pm     
      ASSERT_TRUE(rc == 1, "");
      usleep(2000000);
       
      rc = modbus_read_registers(ctx, (7),
    	                               1, tab_rp_registers);     //Register I/O8-->>gives 1. Green Total[15:8] , 2.Yellow Total[7:0]   	
      Data_buffer[regis_count]=tab_rp_registers[0];              //Register I/O7-->>gives 1.Red Total[15:8] ,   2.Missed  Total[7:0]
								 //Minimum Delay between Write and read is 2 useconds
      ASSERT_TRUE(rc == 1, "FAILED (nb points %d)\n", rc);   
      printf("SITE SURVEY: 0x%X , addr=%d  \n",tab_rp_registers[0],(7));//for debugging
      usleep(160000);
...
//SECTION:-3 (READ SITE SURVEY PERAMETERS) END FROM HERE. 
//=============================================================================================================================================================



    while(1);
 //  memcpy(Data_buffer,tab_rp_registers,50);
   printf("DATA BUFFER READING========\n");//for debugg ing 
  for(regis_count=0;regis_count<50;regis_count++)
  {
	  printf(" Data : %d , Address=%d  \n",Data_buffer[regis_count],9050+regis_count);//for debugging    
  }  
  printf(" counter=%d\n",regis_count); 
  
/*//===============================================================================================
    // Single register //
    rc = modbus_write_register(ctx, 262144, 0x05);   /// actual
   // rc = modbus_write_register(ctx, UT_REGISTERS_ADDRESS, 0x08);     /// changed by pankaj
    printf("1/2 modbus_write_register: ");
    ASSERT_TRUE(rc == 1, "");
    printf("MODBUS WRITE TO REGISTER :SUCCESSFULL\n");//for debugging

 // for(regis_count=0;regis_count<=10;regis_count++)
 // {
    rc = modbus_read_registers(ctx, (262144),
                               10, tab_rp_registers);
    //Data_buffer[regis_count]=tab_rp_registers;
    printf("\nvalue rc: %d conter val=%d \n",rc,regis_count);//for debugging
    for(regis_count=0;regis_count<=10;regis_count++)
    {
    printf("\nValue_readed=0x%X\n",tab_rp_registers[regis_count]);
 ...
 //   usleep(400000);    
  ...
  for(regis_count=0;regis_count<=10;regis_count++)
	  printf("\ buffer value: %d counter=%d  \n",Data_buffer[regis_count],regis_count);//for debugging
//===========================================================================================*/    
  //  printf("\nvalue rc: %d\n",rc);//for debugging
    
   ASSERT_TRUE(rc == 1, "FAILED (nb points %d)\n", rc);
    
 //   ASSERT_TRUE(tab_rp_registers[0] == 0x17, "FAILED (%0X != %0X)\n",
 //               tab_rp_registers[0], 0x17);// actual
     
   while(1);
//    printf("\n MODBUS READING : SUCCESSFULL \n");//for debugging
//    printf("\nValue_readed=0x%X,Value_readed=0x%X\n",tab_rp_registers[0],tab_rp_registers[1]);
    // End of single register //
     printf("===============HOLDING REGISTER READ/WRITE : FINISHED===========\n");


//============MANY REGISTER START HERE=============================================================
/*    // Many registers //
 //   rc = modbus_write_registers(ctx, UT_REGISTERS_ADDRESS,
 //                               UT_REGISTERS_NB, UT_REGISTERS_TAB);//OLD
                                
     rc = modbus_write_registers(ctx, 0X40506,
                                UT_REGISTERS_NB, UT_REGISTERS_TAB);//NEW
                             
    printf("READ MANY REGISTER\n");                            
    printf("1/5 modbus_write_registers: ");
    ASSERT_TRUE(rc == UT_REGISTERS_NB, "");

  //  rc = modbus_read_registers(ctx, UT_REGISTERS_ADDRESS,
  //                             UT_REGISTERS_NB, tab_rp_registers);//OLD
                               
    rc = modbus_read_registers(ctx, 0X40506,
                               UT_REGISTERS_NB, tab_rp_registers);//NEW
  ...
  //  rc = modbus_read_registers(ctx, UT_REGISTERS_ADDRESS,
  //                             0, tab_rp_registers);//OLD
                               
   rc = modbus_read_registers(ctx, 0X40506,
                               0, tab_rp_registers);//NEW
   printf("2/5 modbus_read_registers:READ SUCCESS\n");//FOR DEBBUGGING
    ...
    nb_points = (UT_REGISTERS_NB >
        ...
        UT_REGISTERS_NB : UT_INPUT_REGISTERS_NB;
    memset(tab_rp_registers, 0, nb_points * sizeof(uint16_t));
    printf("====3/5 MODBUS MULTIPLE REGIS READING SUCCESSFULL.\n=====");//for debugging
    ...
    rc = modbus_write_and_read_registers(ctx,
                                         ...
                                         UT_REGISTERS_NB,
                                         tab_rp_registers);
     printf("====3/5 MODBUS MULTIPLE REGIS READING SUCCESSFULL.123\n=====");//for debugging                                    
